...
const peerIdKeys = await peerIdFromKeys(Buffer.from(peerIdObject.publicKey.data), Buffer.from(peerIdObject.privateKey.data))
...
const node = await createLibp2p({
  ...
  dht: kadDHT(),
    peerDiscovery: [
        bootstrap({
        interval: 60e3,
        list: ['/ip4/182.191.70.215/tcp/4001/p2p/12D3KooWEiUjPkRtEJ2tfkRSH7eYKvCPdCf64FFLSN1C9ce6rXoj']
        })
    ]
});
...
// node.start();
console.log("node.getPeers() =====", node.getPeers());
